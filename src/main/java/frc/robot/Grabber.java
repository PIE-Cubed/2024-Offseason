...
public class Grabber {
    ...
    private final int GRABBER_MOTOR3_CAN = -1;    // Grabber motor on the arm
    ...
    private Grabber() {        
        ...
        // Remove this when you get the arm grabber's CAN ID
        // You should be able to find it on Demeter, in motors.txt (on desktop I think)
        // Check if it needs to be inverted (shouldn't but IDK)
        if(GRABBER_MOTOR3_CAN == -1){
            System.err.println("GRABBER MOTOR ERROR, please set the arm grabber's motor ID (GRABBER_MOTOR3_CAN). See comment on line 65 of Grabber.java");
        }
        grabberMotor3 = new CANSparkMax(GRABBER_MOTOR3_CAN, MotorType.kBrushless);
